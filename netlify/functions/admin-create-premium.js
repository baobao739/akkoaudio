...
exports.handler = async (event) => {
  ...
  try {
    ...
    try {
      ...
      return json(500, { error: e.message || "Supabase not configured", hint: diagnose().issues });
    }
    ...
    for (let i = 0; i < count; i++) {
      ...
      for (let attempt = 0; attempt < 8; attempt++) {
        ...
        if (/relation .* does not exist/i.test(msg) || error.code === "42P01") {
          return json(500, {
            error: "Table premium_codes missing. Run supabase-premium.sql in Supabase."
          });
        }
        if (/invalid api key/i.test(msg)) {
          return json(500, {
            error: "Invalid API key — check SUPABASE_SERVICE_ROLE_KEY in Netlify."
          });
        }
        return json(500, { error: "Could not create premium code: " + msg });
      }
      if (!inserted) return json(500, { error: "Could not create unique premium code." });
    }
